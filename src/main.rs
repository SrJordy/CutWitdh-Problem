use rfd::FileDialog; // Importamos la biblioteca rfd para mostrar una ventana de diálogo para seleccionar archivos.
use crate::Cargar_Archivo::CargarArchivo; // Importamos la función CargarArchivo desde el módulo Cargar_Archivo, que se encargará de cargar el grafo desde un archivo.
use crate::Heuristica::heuristica; // Importamos la estructura heuristica desde el módulo Heuristica, que implementa la lógica para encontrar la mejor solución.
use std::time::Instant; // Importamos Instant para medir el tiempo de ejecución del proceso.

mod Cargar_Archivo; // Declaramos el módulo Cargar_Archivo, que contiene la lógica para cargar archivos de grafos.
mod Grafo; // Declaramos el módulo Grafo, que define la estructura del grafo y sus nodos.
mod Calcular_Cutwidth; // Declaramos el módulo Calcular_Cutwidth, que contiene la lógica para calcular el cutwidth de un grafo.
mod Heuristica; // Declaramos el módulo Heuristica, que contiene la lógica para generar soluciones y encontrar la mejor.

fn main() {
    // Abrimos una ventana de diálogo para que el usuario seleccione el archivo de grafo.
    let ventana = FileDialog::new() // Creamos una nueva instancia de FileDialog.
        ...
        .pick_file() // Abrimos la ventana de diálogo para que el usuario seleccione un archivo.
        .expect("Ocurrió un error al abrir el archivo"); // Si ocurre un error al abrir el archivo, mostramos un mensaje.

    // Obtenemos la ruta del archivo seleccionado y la convertimos a una cadena de texto.
    let ruta = ventana
        .to_str() // Convertimos la ruta del archivo a un string.
        .expect("Ocurrió un error al convertir la ruta a string"); // Si ocurre un error al convertir la ruta, mostramos un mensaje.
